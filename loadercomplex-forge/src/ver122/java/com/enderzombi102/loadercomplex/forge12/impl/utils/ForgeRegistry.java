...
public class ForgeRegistry implements Registry {
	...
	public static CreativeTabs getOrCreateCreativeTab( @Nullable ResourceIdentifier itemGroup, ResourceIdentifier icon ) {
		if ( itemGroup == null ) {
			return null;
		}
		if ( !ITEM_GROUPS.containsKey( itemGroup ) ) {
			LoaderComplex.get().getContext().getRegistry().registerCreativeTab( null, icon );
		}
